...
const randVec = () => Uint8Array.from({length:4},_=>Math.random()*256);
...
(async () => {
  ...
  log('SQLite', version.libVersion,
      '| sqlite-vec', version.vecVersion /* added by vec0 */);
  ...
  const rows = (await promiser('exec',{
      dbId, sql:'SELECT count(*) AS n FROM v'})).result.rows[0].n;
  if(!+rows){
    const stmt = await promiser('prepare', {
      dbId, sql:'INSERT INTO v(rowid,vector) VALUES(?1,?2)'
    });
    for(let i=1;i<=3;++i)
