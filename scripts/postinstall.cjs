...
if (!ok) {
  console.warn(
    '[sqlite-vec-wasm] Missing wasm payload. Reinstall or run `npm run build`.',
  );
} else {
  ...
  try {
    ...
    const hasNative =
      ...
      fs
        .readdirSync(outDir)
        .some((f) => /sqlite-vec.*\.(so|dylib|dll)$/i.test(f));
    if (!hasNative && process.env.SQLITE3_VEC_PREBUILT !== '0') {
      ...
      if (res.status !== 0) {
        console.warn(
          '[sqlite3-vec] Prebuilt download not available (optional).',
        );
      }
    }
    ...
    if (!hasNative && optIn) {
      ...
      if (!vecDir || !fs.existsSync(vecDir)) {
        console.warn(
          '[sqlite3-vec] Skipping native sqlite-vec build: set SQLITE_VEC_DIR to the sqlite-vec repo path.',
        );
      } else {
        console.log(
          '[sqlite3-vec] Building native sqlite-vec extension from',
          vecDir,
        );
        ...
        if (res.status !== 0) {
          console.warn(
            '[sqlite3-vec] Native sqlite-vec build failed (optional).',
          );
        } else {
    ...
    console.warn(
      '[sqlite3-vec] Postinstall native build skipped:',
